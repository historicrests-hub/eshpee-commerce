...
export default function CartModal() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      const response = await fetch('/api/payment/priyo-pay', {
        method: 'POST',
        headers: {
      ...
      if (paymentLink.payment_link_url) {
        // Redirect to Priyo Pay payment link
        window.location.href = paymentLink.payment_link_url;
      } else if (paymentLink.redirect_url) {
        window.location.href = paymentLink.redirect_url;
      } else {
        toast.error('Invalid payment link response');
        setLoading(false);
      }
    } catch (error) {
